...
from exoplanet_astronomer import ExoPlanet, get_all_system_names, get_system, update_exoplanets_with_min_date, post_new_exoplanet_system, render_system
...
async def random_art_html(request: Request, art_type:str|None = Query(None, max_length=200, pattern="^[a-z]+$")):
...
async def random_nasa_html(request: Request, search_term:str|None = Query(None, max_length=200, pattern="^[a-z]+$")):
...
@cache(expire=60*5)
async def random_met_object(art_type:str|None = Query(None, max_length=200, pattern="^[a-z]+$")) -> dict:
...
async def random_nasa_image(search_term:str|None = Query(None, max_length=200, pattern="^[a-z]+$")) -> dict:
...
@internal_api.get('/emoji_system', response_class=PlainTextResponse)
async def emoji_system(host_star: str) -> str:
    background_tasks = BackgroundTasks()

    system = await exoplanetary_system(host_star=host_star, background_tasks=background_tasks)
    if system and type(system) is list:
        return render_system(system=system)
